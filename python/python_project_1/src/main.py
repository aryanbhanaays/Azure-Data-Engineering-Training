...
def calculate_average_costs(items_file, sales_file, output_file):
    """
    Calculate the average cost of items sold within their valid date ranges and save the results to a CSV file.
    ...
    filtered_df['TotalSalesValue'] = filtered_df['NumSales'] * filtered_df['Price']
    
    avg_cost_df = filtered_df.groupby('ItemId')['TotalSalesValue'].sum() / filtered_df.groupby('ItemId')['NumSales'].sum()
